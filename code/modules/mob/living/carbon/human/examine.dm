...
//		msg += ", <b><font color='[species.get_flesh_colour(src)]'> \a [species_name]!</font></b>"
	...
	if(!skipface)
		if(branded) //For brands.
			msg += "<span class='warning'><b>[src.name] ИМЕЕТ КЛЕЙМО \"[branded]\" НА ЛИЦЕ!</b></span>"
			if(ishuman(user))
				var/mob/living/carbon/human/H = user
				if(H.religion == IDEOLOGY_MONARCHISM && H != src && branded == "НАЦИСТ")
					msg += "<span class='notice'> Наблюдение подобного доставл&#255;ет вам удовольствие.</span>"
					H.add_event("punishednazi", /datum/happiness_event/punished_nazi)
				if(H.religion == IDEOLOGY_MONARCHISM && H != src && branded == "КОММУНИСТ")
					msg += "<span class='notice'> Наблюдение подобного доставл&#255;ет вам удовольствие.</span>"
					H.add_event("punishedcommie", /datum/happiness_event/punished_commie)
				if(H == src)
					msg += "<span class='notice'> Это напоминает мне о соде&#255;нном.</span>"
	...
	if(gender == FEMALE && !wear_suit)
		msg += "<B>Размер груди: [tits].</B>\n"
